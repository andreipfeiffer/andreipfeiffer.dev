...
One of my main struggles in software development over the past 20 years has been organizing the files and folders of software projects. To put it simply: I always found it hard to figure out 1) what files should stay in the same folder and which ones should be separated; 2) when should I create a new folder and what files should go into it; 3) when to split a file into multiple ones; 4) when to combine multiple files into a single one. Sounds familiar?

There are various general approaches, such as organizing files either by **type**, **layer** or **domain**. You can read more about each approach in [Folder structure for big projects: package by type, layer or feature?](https://labs.madisoft.it/folder-structure-for-big-projects-package-by-type-layer-or-feature/).

For a long time I thought that choosing one of the above approaches is simply a matter of preference. Not to mention that online content also suggests there is one solution better than the others, only because it works better for the author. However, recently I gave it a lot of thought and I came to the conclusion that all approaches are **equally viable**, but at different **scales of a software project**.
...
Towns usually range from several thousands up to a few tens of thousands of inhabitants. Most of them also have their own hospital, with general practice doctors and specialized ones as well. Larger hospitals might even include specialized clinics inside the same building.
...
It's worth noticing that it comes naturally to place all the files of a certain type within the a folder, naming it using the type of the files within. For instance, putting all the `*.service.*` related files into a `/services` folder.

At small or medium scale this approach works well, however when we increase the scale even further we encounter an issue. When implementing a new feature or changing an existing one, we usually work on a low number of domains, sometimes touching a single one. If the files are organized by type, this implies searching in different folders for the files related to the same domain.
...
It's more beneficial for the citizens when the cities have their own hospitals. On the other hand, it's also a security measure. In case one hospital stops being operational, for whatever reason, there are others that might take care of the patients.

It is also more scalable, since different cities have different needs. They are free to self-manage. Smaller cities might have a single hospital. Large cities could have multiple hospitals specialized in the same discipline.
...
Ok, but what about even larger scale? Is there any other approach that works better? Well, not if we look at global hospitals distribution. Regardless of national, continental, or global scale, the hospitals are built to serve the needs of a single settlement. They are never concentrated into a single area to serve a large area of multiple cities.
...
- It's often not obvious **in which domain to place certain files**. For instance files concerning `BillingInformation` should be placed in the `UserAccount` domain, as there is a one-to-many relationship between the 2? Or should it be placed in `Subscription` domain because there's also a one-to-one relationship between them?
- It's debatable **when to create a new domain**, or when to extract files from an existing domain to a new one. For instance, `BillingInformation` could be extracted in it's own domain folder. But what about something like `UserPreferences` that's tightly coupled with `UserAccount`? There's usually a grey zone when deciding to extract.
- Where shall we place **code shared by very few domains**, that doesn't make sense to stay in its own domain? We could place it in a separate `Shared` domain, but that might end up bloated up.
...
We could **combine packaging by layer, by type, and by domain** at different levels. In the following example, we'll look at a single page application.
...
- `/pages` folder represents the outer layer, containing the page specific components assigned to the routes, along with any non-reusable components and logic;
...
Going deeper into the layers, the `/modules` folder follows a **groupping by domain** structure, because here is where we encapsulate the core of our domain code. Each subfolder will encapsulate the logic of each entity of our domain:
...
We could include abstract or application-wide services, helpers and utils, constants, hooks, types, wrappers of 3rd party libraries, you name it.

<Note title="Important Note">
  <p>The example above follows an eclectic mix of principles and methods discussed
  in this article. It is not meant to provide a solution for any software
  project. However, if it fits your needs, you can use it as is, or get some
  inspiration for your own project.</p>

  <p>To be noted that it is not only a theoretical example. The above structure is used in a fairly large project, with 4k files totalling 700K lines, written over the course of 10 years.</p>
</Note>

## To conclude

Organizing project files is not different than the distribution of any other public service that we use in real life. The only difference is the size of the location that it serves.

To synthesize everything we talked about:
...
However, in real life we would most likely adopt a **hybrid approach**, combining multiple methods that makes sense to address the needs of our own projects and our own development team(s).
