...
The above code is an example of a Dockerfile which defines a MariaDB server and a Redis service. Even if we not familiar with the syntax, this file is easy to read and understand the presequisites of the project.

## Constraints

Some projects require a specific **Node.js** version. We could mention the required versions in the `README.md` file, but it's likely that it will be overlooked, especially if we have multiple projects with different required versions.
...
Now, when we switch between projects, we can run `nvm use` and it will automatically switch to the specified Node.js version.
