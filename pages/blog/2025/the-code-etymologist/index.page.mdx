...
There is a general belief that spoken [languages](https://en.wikipedia.org/wiki/Origin_of_language) emerged more than a hundred thousands of years ago. Even though the [first writings](https://en.wikipedia.org/wiki/History_of_writing) were invented around 3000 BC, [formal education](<https://en.wikipedia.org/wiki/History_of_education#Formal_education_in_the_Middle_Ages_(500%E2%80%931500_AD)>) was introduced much later, in the Middle Ages. However, it was limited to churches, monasteries, and the upper class.
...
When was the last time you heard, or even said it yourself: _“There is no documentation”_? I have started hearing and saying it myself once I began working on _large_ projects, that spread over multiple years and having multiple team members.

So, we have to ask ourselves **_"Why is there no documentation?"_**  
The obvious answer is that **_"Because nobody wrote it."_**

But, **_"Why haven't anybody wrote it by now?"_**  
Well, **_"Because it takes time and effort."_**

If _it works_ without it, why bother, right? However, the cost will be eventually paid later, with interest. Team members, new or old, will eventually search for answers. They will have to find the right person, which will have to recall the details or even rediscover lost knowledge. This too requires time and effort. The difference is that this scenario will occur multiple times.
...
So, there is no clear role responsible for writing internal documentation. It is just one of the various skills that **software developers** should have. And these skills overlap to a large extend with the responsibilities of a dedicated technical writer.
...
- **Graphics** provide a more intuitive alternative to text, that helps us visualise information in a concise manner. Graphical approaches are best suited for documenting <LinkTo part={3}>development workflows</LinkTo>, or to explain how a system works using <LinkTo part={4} hash="flow-charts">flow charts</LinkTo> or <LinkTo part={4} hash="sequence-diagrams">sequence diagrams</LinkTo>.
...
- **Tools** are great at enforcing practices for all tam members to ensure a consistent development process. Such practices include <LinkTo part={2} hash="code-formatters">code formatting</LinkTo>, <LinkTo part={2} hash="linters">linting</LinkTo>, or <LinkTo part={1} hash="constraints">constraining</LinkTo> a specific Node.js version.
- **Interactive** interfaces provide a whole new level of documentation, which is helpful both for visual elements such as <LinkTo part={5}>UI components</LinkTo> or design systems, but also to non-visual systems such as <LinkTo part={4} hash="state-charts">state charts</LinkTo>.
...
If we don't get passed the speaking method, it is certain that someone will eventually become a code etymologist, doomed to search for lost or opaque information.
