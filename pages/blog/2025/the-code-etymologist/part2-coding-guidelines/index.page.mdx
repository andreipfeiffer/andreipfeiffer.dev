...
The question is, _"How can we discover these guidelines as new team members?"_. Well, if there's no written documentation, we'll probably discover them during **code reviews**.

To avoid repeting the same comments to different team members, writting down repeating anti-patterns, along with explanations to the problem and solution exemplification can become the **internal coding guidelines document**. Some examples of such documents include:
...
- On our [continuous integration](https://prettier.io/docs/ci) solution, to either verify that the code is properly formatted or perform automatic formatting as well. This centralized approach offers the highest degree confidence to prevent malformed code being merged.
...
- **Preventing deprecated imports** is a useful technique, especially during code migrations. The [`no-restricted-imports`](https://eslint.org/docs/latest/rules/no-restricted-imports) rule allows us to define which forbidden imports and provide custom messages to instruct team members what to do instead.
...
https://bitbucket.org/ateamo/smilecloud-frontend/src/develop/.vscode/extensions.json
