...
If we think of a UI Component as a _"function of props"_, we could **render components in isolation**, outside the application that uses them.
...
So, the technique is required in this situation because we're implicitly developing the components in isolation.
...
Having all the component states rendered in a separate UI provides excellent documentation for anyone working on the project, quickly discovering the existing components and their usage. It also helps avoid reinventing the wheel and creating similar components, prevalent in lengthy projects with larger teams.
...
- see the code that produces a specific component state;
