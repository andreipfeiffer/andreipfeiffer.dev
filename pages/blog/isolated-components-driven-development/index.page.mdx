...
Isolated components driven development is a technique I've been using for years, during development or debugging, which proved to be incredibly useful and even necessary in some particular scenarios.
...
If we think of a UI Component as a _"function of props"_, then we could **render components in isolation**, outside the application that uses them.
...
So, in this situation, the technique is required because we're implicitly developing the components in isolation.

<Subheading>Working with read-only UIs</Subheading>

Typically, most UIs include all [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete) operations. That allows us to add data, which is needed to build UI components during development. Without the data, we could only render the empty states.

<Figure background={"#fff"}>
  <Image src={crud_ui} width="1396" height="633" />
</Figure>

However, some UIs, like Google Analytics charts or Github diff view, are primarily read-only. They read and display existing data, but we cannot add new data from the UI. The source of the data is different than the destination that displays it.
...
A more convenient solution would be to develop the UI components in isolation, allowing us to bypass any data sources and render them effortlessly with any arbitrary props and in any desired state.

<Subheading>Working with unstable APIs</Subheading>

There are multiple methods to [connect the UI to their web API](/blog/developing-uis-connected-to-web-apis), each having some strong points and limitations. But sometimes, the APIs that we integrate with might be unstable for various reasons: they might be under development, changing frequently, or being offline when we need them most.
...
Let's consider the favorable scenario. We don't have to deal with a read-only user interface. We're not building a components library, and we're working with a very stable API.

<Figure background={"#fff"}>
  <Image src={component} width="1831" height="858" />
</Figure>
...
4. passed down the data to descendent components and trigger some events;
...
To begin with, we don't necessarily need special tooling for this technique. For simple applications, we can use separate page(s) to render the required components. For example, I hadn't built a single page of this website before all the required building blocks were implemented and rendered in this [**style guide**](/style-guide) page.

However, for larger applications, we would probably need more features, and there many existing solutions that we can choose from. I've personally used two different libraries in the past with great success:
...
As we saw, isolating components is beneficial during development and debugging. But there are additional valuable benefits that worth mentioning.

<Subheading>Documentation</Subheading>

Having all the component states rendered in a separate UI provides excellent documentation for anyone working on the project, allowing them to quickly discover the existing components and their usage. It also helps avoid reinventing the wheel and creating similar components, which might be prevalent in lengthy projects with larger teams.
...
Fortunately, dedicated [tools](#tooling) generate an interactive UI where we can:
...
A common pattern is to decouple the **component that fetches** the data, typically called a smart component or a container, from the **component the displays** the data, typically called a dumb or a presentational component.
...
<Subheading>Focus on the Component's API</Subheading>
...
To see the city skyline, we'd have to go out of the city. Likewise, to see what the Matrix is, we'd have to step out of the Matrix. The same applies to component API design.
