...
- passing a dummy object in your tests, etc.

<br />

There is a straighforward way to do this:

1. **define the type** first;
2. then **initialize the object** based the previously defined type.
...
So let's examine a couple of alternative approaches, that might help us with the problems mentioned above.
...
One quick and dirty solution to our problem is to trick the type system to accept an empty object as the needed type. As tempting as it might be, because of how easy it is to use it, **this approach introduces some huge problems**:
...
You might find yourself polluting your code with defensive methods, like optional chaining, to avoid runtime errors:
...
There is another approach we can use, that was presented to me by [Martin Hochel](https://twitter.com/martin_hotell). Instead of defining the type first, and then perform the initialization, we can do it the other way around:
...
There are of course some limitations when you have to deal with more specific types.
...
In this case, using a type assertion like **`country: EMPTY_COUNTRY as Country`** won't work, as the type system will not infer the nested type as expected (at least not in TypeScript 4.1).

This limitation is not a functional problem, but more of an esthetic issue, that impacts the way you vizualize the type in your code editor. Seeing the entire list of properties can obscure the overview of complex types with many properties or nested structures.
...
However, this does not work with custom types, that are created using the `typeof` operator. This is probably a consequence of limitation 2, previously mentioned:
