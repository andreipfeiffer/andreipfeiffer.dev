...
As Wikipedia states, Technical Debt is a trade-off, a choice we make as developers. Sometimes we are (or feel) compelled to make
these trade-offs. Sometimes it's our own decision. The fact is we can't always avoid them, so we need to learn how to manage them.
...
We should all aim towards writing well-designed code, even if we are only able to reach a good enough level.

Needless to say, it's the poorly designed code that we should stay away from. It has a tendency to [rot](https://en.wikipedia.org/wiki/Software_rot), which can potentially make our code unsalvageable.

## Why do we write poorly designed code?

I'm pretty sure nobody writes "bad code" on purpose. There are various reasons why we do this, and next, we will cover the ones that are understandable. Negligence and not giving a damn about consequences are not among them.
...
- whenever we have to use a lot of words in order to explain to a colleague a piece of code that we wrote, it could be a sign of **opacity**, so we need to make the code [more readable](https://blog.pragmaticengineer.com/readable-code/);
...
trade-off. To talk about debt, we have to be aware of better, more robust approach, but decide to choose a quick & dirty solution instead. If we don't know a better solution, we're basically implementing the right solution, be it a poor one.
...
Through experience, we'll be able to understand trade-offs, and we’ll probably feel the urge to fix them. We might even:
...
Eventually, this will lead to [broken windows](https://blog.codinghorror.com/the-broken-window-theory/), which will incur Technical Debt. We might even believe that "we did our part", because we have acknowledged the problem, and now it's time for somebody else to prioritize the refactoring.

But acknowledging a problem, and not fixing it, is never a solution. Sooner or later, we'll learn it the hard way.
...
developers, need to understand that we are the ones that make that choice.
...
know will become big problems later on, for the development team and consequently for the product team.
...
money from a friend for a short period of time. You'll still need to return the money, but you won't pay any additional interest.
...
Always keep a balance between refactoring the existing code, and releasing new features:

- allocate 1-2 team members to focus on the refactoring, while the rest of the team develops new features;
- if the entire team is required to focus on the refactoring, then allocate only part of the sprint for it, and the other part for new features.
