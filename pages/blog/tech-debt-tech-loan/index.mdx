...
I'm pretty sure nobody writes "bad code" on purpose. There are various reasons why we do this, and next, we will cover the ones that are understandable and skip _negligence_ or _not giving a damn_.
...
However, none of the above helps if we don't actually _fix the problem_. Even worse, if the trade-off "works", we might be forced to
...
> Acknowledging a problem, but not fixing it, doesn't really solve anything.

Sooner or later, we'll learn it the hard way.
...
money from a friend for a short period of time. We'll still need to return the money, but we won't pay any additional interest.
