...
This chapter examines **Atomic CSS**, a fundamentally different approach, which in contrast to Semantic CSS, employs non-semantic classes that usually contain a single CSS rule.
...
The motivation for Atomic CSS is obvious: _"avoiding the headache of writing and managing semantic stylesheets"_ due to global scope, complex selectors, specificity problems, and source code duplication. To avoid all these problems, Atomic CSS entirely contradicts Semantic CSS.
...
- We can only use the above `large-title` class when we want the text content to be both **bold & green**. For instance, to have a different title in **bold & red**, we would have to create a new class.
- The more CSS rules we add to a CSS selector, the more specific it becomes, creating a tight coupling between the styles and their usage. Highly specific and tightly coupled implementations are generally less reusable.

<br />

Since semantics specifically describe the content, their reusability is typically limited. Therefore, we would have to adopt a non-semantic mindset to benefit from high reusability.
...
- **Composition** because we can combine these CSS classes in any way we want. Think of them as lego blocks or Subway ingredients, which we can combine in countless ways.
- **Immutability** as we'll never override (aka mutate) defined CSS classes. A `.bold` class should always have the `font-weight: bold;` implementation.
...
<Note>
  Some exceptions to <strong>purity</strong> include theming and media queries,
...
- **simple utilities** containing a single CSS rule for [color](https://getbootstrap.com/docs/5.1/utilities/colors/), [spacing](https://bulma.io/documentation/helpers/spacing-helpers/), [typography](https://getuikit.com/docs/text), etc, as most semantic CSS frameworks include.
...
To begin with, we don't write CSS when new styling is required. Instead, we define most CSS classes upfront, taking all the required combinations into account. So, for instance, considering text alignment options, we'll define several CSS classes:
...
Since all the required CSS code is defined beforehand, we **won't be adding new CSS code** in the long term. Instead, we will only reuse existing CSS classes. Thinking about it, this means that most **maintenance and scalability CSS issues simply disappear**.
...
Many frameworks implement Atomic CSS approaches, but only a handful became popular. Since there are some subtle and interesting differences between them, let's take a closer look.

<Subheading>BASSCSS</Subheading>

According to Github Insights, the first Atomic CSS framework, released in 2013, is [**BASSCSS**](https://basscss.com/). It contains utilities only for layout, spacing, and typography, making it is more suited for design and rapid prototyping. In addition, colors and element styling are available as [add-ons](https://github.com/basscss/addons).
...
[**ACSS**](https://acss.io/frequently-asked-questions.html) or **Atomized CSS** was also released in 2014. Instead of using a predefined set of classes like the other frameworks, it included a build step to generate only the CSS classes that are actually used.
...
2. **Descendant and child combinators are not supported.** It's true that in large applications, it's generally discouraged using such selectors, but they might be required in specific scenarios.
3. **Debugging CSS code in browsers' devtools is cumbersome**, as we have only one style rule per CSS class.
4. Some argue that Atomic CSS **bloats the HTML markup** during Server-Side Rendering (SSR) because we're _simply moving code from CSS to HTML_, thus breaking the separation of concerns.
...
Atomic CSS frameworks are also called **utility-first** frameworks. If we analyze this alternative definition, it doesn't convey that we _"shouldn't write a single line of CSS code ever again"_.
...
Whether we like it or not, the truth is that Atomic CSS solves most of the problems of semantic CSS, namely code duplication, specificity, increasing code output, or complex selectors, just to name a few. Therefore, Atomic CSS proves to be a valid alternative to Semantic CSS, bringing countless benefits to the table.

<br />

In the following chapter, **<LinkTo part={CURRENT_PART + 1} />**, we'll explore a novel approach to CSS, by moving style definitions to JavaScript files. As a result, we benefit from **explicit dependencies** and trivial **variable sharing**. In addition, CSS-in-JS provides all the features of styles encapsulation, CSS preprocessors, Atomic CSS, and so much more.
