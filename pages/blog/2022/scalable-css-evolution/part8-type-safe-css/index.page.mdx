...
Without type safety, extracting a component and its styles into a separate file is a daunting task. That might be the reason why we're reluctant to perform such refactorings.
...
During the past years, static type checkers like TypeScript have a steadily increasing adoption in web development, allowing us to write **type-safe business logic**. Additionally, TSX enables **type-safe JSX markup**, being officially supported by TypeScript. Finally, CSS-in-TypeScript provides **type-safe styles**.
