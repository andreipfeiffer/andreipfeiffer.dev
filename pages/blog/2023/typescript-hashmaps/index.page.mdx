...
// 3️⃣ dynamic member access
const A = hashMap.get("A"); // 👍 boolean | undefined
const X = hashMap.get("X"); // ✅ boolean | undefined
```
...
Notice that I've used `.set()` method to add new members after instantiating a `new Map()`. The constructor can receive initial values sa well, by passing an [Array of key-value tuples](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map#relation_with_array_objects):
...
We can also convert a plain Object, by using `Object.entries()` since it returns an `Array<[key, value]>`, which is exactly what the Map constructor needs:
