...
During this part, we'll continue our journey to overcome the problems with CSS maintainability, focusing on **avoiding complex CSS selectors and specificity wars**. Therefore, let's explore some essential good practices introduced by various thought leaders:
...
<!-- prettier-ignore-start -->
```css
/* ❌ could be potentially overridden */
.header {}
...
It happened to me so many times to think that _"we'll never have more than one Contact Form"_. But one week later, we needed to add a _Subscribe Form_, which had the same look as the _Contact Form_.
...
- for some reason, somebody needs to replace the `h2` with an `h3`, which is a trivial change and it shouldn't break anything;
...
<!-- prettier-ignore-start -->
```css
/* ❌ avoid HTML tag names */
.card h2 {}

/* ✅ classes are way more flexible */
.card .title {}
```
<!-- prettier-ignore-end -->
...
- Some of the **good practices don't apply equally** to every project, with every team. Also, developers tend to be very opinionated, so we rarely reach a consensus regarding a specific set of rules.
