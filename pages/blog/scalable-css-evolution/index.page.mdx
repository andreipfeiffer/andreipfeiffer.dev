...
import img from './scalable-css-evolution.png'
...
I've been writing Cascading Style Sheets (CSS) since 2000. At that time, CSS level 1 was implemented in browsers. The current standard in 2021 is CSS level 4. As you can imagine, a lot has changed since then regarding what we can achieve nowadays with CSS.

One thing I came to understand after all these years is that there's a significant difference between the CSS code writen for **small, mostly static websites** and the one developed for **large, highly dynamic applications**. Single developers working on a project that will end in a few months can write CSS in any way they consider appropiate. Poorly written code will probably not impact the success of the project.
...
Before 2000, the World Wide Web Consortium (W3C) set the tone for CSS features, releasing standard specifications way before their actual implementations in browsers. But with the rise of Web 2.0 things began to flip. CSS standard features started to lag behind the needs of the community. Therefore, developers had to **build their own tools** to aid them in writing CSS at scale.

<br />

You can view this series as a **chronicle**, describing how CSS tools and techniques have evolved over time. The goal is to present an objective view of the current state of writing _maintainable CSS at scale_, highlighting the available tools and their applicability. This chronicle should be appealing to you if:

- You are **curious to discover** potential maintainability problems when working with CSS at scale.
- You have already experienced the problems with CSS, and you're **looking for solutions** to overcome them.
- You've **heard certain buzzwords** such as _specificity_, _semantics_, _scoping_, _encapsulation_, _Atomic CSS_, _CSS-in-JS_, _design tokens_, _type-safety_, etc, but you don't fully grasp their meaning.
- You want to better **understand the whole ecosystem**, even if you master a few particular tools.
- You're **looking forward to see** what CSS might look like in the near future.
- You're simply **eager to learn** _how_ CSS tooling have evolved over time or _why_ certain techniques exist today.
...
<Figure background={"#fff"} width="grid">
  <Image
    ...
    width="1920"
    height="1080"
    alt="Timeline of scalable CSS evolution, depincting Atomic CSS and CSS-in-JS branch off from Semantic CSS"
  />
</Figure>

In the following articles we'll analyze all the tools and techniques that are still applicable today in regard to scalable CSS code. Here's a short overview of what we'll cover.
...
- **<LinkTo part={3} />** walks through some of the essential techniques introduced by various pioneers, to **avoid naming collisions** and **specificity wars**. It's worth noting that these practices are applicable with or without the help of CSS processors.
...
- **<LinkTo part={5} />** debates an alternative approach that contradicts and breaks all the principles of Semantic CSS. This shift gave birth to a whole new set of frameworks based solely on _single-purpose utility classes_.

- **<LinkTo part={6} />** focuses on the tools used for CSS scoping, addressing all the problems regarding **naming collisions**, **specificity wars**, **source order precedence**, and reducing the amount of **zombie code**. Styles encapsulation became an industry standard, adopted by all component-based JavaScript frameworks.
...
- In **<LinkTo part={8} />**, we'll see how combining TypeScript with CSS-in-JS adds type checking for CSS, enabling _safe refactorings_ and _typed interfaces_ when authoring UI components. Therefore, CSS-in-TypeScript provides the missing puzzle piece when aiming for a complete **type-safe code** base.
...
There's a tight relationship between _scalability_ and _maintainability_. That's why I'll often use these terms interchangeably throughout this series. Any scalability concern could potentially affect the maintainability of the code.
