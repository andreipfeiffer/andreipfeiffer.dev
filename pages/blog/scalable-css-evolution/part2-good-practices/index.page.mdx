...
```css
/* ❌ BAD: overspecific selector (0.4.3 specificity) */
...
- If there is a single `.main_menu` element on the page, then the `.header` selector is unnecessary. In case there are more than one `.main_menu`, we could use a different class name, like `.header_main_menu`, to differentiate them.
...
Avoiding the problems mentioned previously is quite simple: avoid targetting HTML tags in CSS and **use classes** instead.
...
- Most practices are typically **impossible to enforce**. The only weapon is constant code review, training, and mentoring.
- Once we have an existing large application written without these practices in mind, they become **tough to introduce onto an existing codebase**. Changing CSS, in general, is a fragile task, so most developers avoid big refactorings.
