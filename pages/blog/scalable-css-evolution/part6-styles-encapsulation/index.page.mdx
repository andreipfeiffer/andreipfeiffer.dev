...
In <LinkTo part={4} />, we've covered various CSS methodologies, including OOCSS, BEM, SMACCS, and ITCSS. They all adopted the Semantic CSS approach, which solves many maintainability problems, but only to some extent.

In this article we'll turn our attention to **styles encapsulation** and **CSS Modules**, which elegantly solve any naming collisions within the global CSS namespace while keeping specificity low at the same time. These techniques profoundly changed the way we author semantic CSS nowadays.
...
Another approach to encapsulate styling that I haven't mentioned here is **CSS-in-JS**. I skipped it because the topic of writting CSS in JavaScript deserves its own part within this series which we'll cover next in <LinkTo part={CURRENT_PART + 1} />.
