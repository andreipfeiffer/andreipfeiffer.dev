...
Before discussing the solutions to CSS scalability issues, we must understand what these issues are in the first place. So, let's take a look at some of the **most concerning problems** when talking about CSS scalability and maintainability:
...
3. [**Specificity wars**](#specificity-wars) when using CSS specificity to override styles;
4. [**Source order precedence**](#source-order-precedence) could render unexpected results;
5. [**Implicit dependencies**](#implicit-dependencies) hinder debugging and code understanding;
6. [**Zombie code**](#zombie-code) increases CSS file size while being tricky to remove;
7. [**Shared variables**](#shared-variables) between CSS and JavaScript code isn't trivial;
...
We'll face an even bigger problem when dealing with responsive web pages and media queries. We can group responsive styles either **by media query** or **by CSS selector**:
...
As developers, we usually try to avoid code duplication as much as possible. That's why some of us prefer to define the media queries only once and include all related styles within that query.

The downside is that it's challenging to read, understand, and maintain such code. The styles for a single element would get split into different parts of the file. **Figuring out which styles apply to a particular selector becomes a scalability problem.**
...
<p>
  In <LinkTo part={2} />, we'll see how tools that support contextual styles
...
Reusable class names usually contain common nouns, like `.modal`, `.button`, `.overlay`, etc. If we include an external file that defines the same class names, they could get overwritten, depending on which stylesheet we include last.
...
It's not unusual to deal with **dynamic** class names, computed using some custom logic. As a consequence, this would prevent us from searching for a string like `.abc-heading-large`:
...
One way to avoid naming collisions is to increase the "strength of a selector" called [specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity), which works in the short term, but it usually gets out of control sooner or later:
...
- <LinkTo part={4} /> teach how we can maintain a low specificity by choosing class
  composition for overwriting;
- <LinkTo part={5} /> covers out-of-the-box solutions using utility classes.

## Source order precedence

Ok, let's say that we stick to low specificity and somehow manage to avoid naming collisions. When specificity is the same, source code order is considered, as _styles declared later will always win_.

Now, let's consider the following trivial example:
...
When looking at the `.css` file, we see that `.red` is defined later, which means that its styles will win because both selectors have the same specificity. Not to mention that the `.red` class could be defined in a separate stylesheet included later in the document.
...
- <LinkTo part={6} /> and <LinkTo part={7} />, on the other hand, offer
  out-of-the-box solutions.

## Implicit dependencies

CSS stylesheets work by default as **explicit dependencies** for HTML because we have to explicitly reference them in the `<head>` part of the document. On the other hand, CSS rules and selectors work by default as **implicit dependencies** for UI components, templates, or any partial markup of an HTML page.
...
<html lang="en">
  <head>
    <!-- "style.css" is an Explicit Dependency for the document -->
...
```js
// "modal" class is an Implicit Dependency for the component
...
- We want to delete the associated styles, but we have no idea if they are used elsewhere within the codebase. So, instead of breaking any existing code, we **choose not to touch existing styles**. Extra styles won't hurt anybody, right?

We avoid deleting _unused CSS code_ because making sure that it really is _unused_ usually implies a great effort.
...
There are various reasons to use custom `values` both in CSS and also in JavaScript:
...
Without going into the technicalities, there are technical solutions to [share variables from CSS to JavaScript](https://www.falldowngoboone.com/blog/share-variables-between-javascript-and-css/), using any aforementioned method to define CSS variables.

<Subheading>Using JS as the source of truth</Subheading>

The alternative is to store the values in JavaScript variables or objects and expose them to (S)CSS. This approach looks more convenient if we think about [Universal Design Tokens](https://udt.design/) (UDT), which also suggests using JSON as an interchangeable data format. And we all know that JSON plays nicely with JavaScript.
...
- **Code editors cannot display suggestions** for defined (S)CSS (Modules) variables, making them difficult to discover especially to developers unfamiliar with the code. Some plugins attempt to support this limitation, but they are pretty flawed.
...
<p>
  ...
  variables with CSS-in-JS approaches. When we define styles in JavaScript
  files, they can reference any JS value.
</p>
