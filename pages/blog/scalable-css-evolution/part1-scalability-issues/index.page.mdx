...
Before going any further to discuss the solutions, we must understand the problems. Generally speaking, when we talk about _software scalability_, we look at **how something behaves when we greatly increase its size**.

To give an example, if a certain problem increases proportionally with the size of the code, it means that we have a **scalability issue**. For instance, we know that we [_should not repeat ourselves_](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself), right? Would it matter if we repeat ourselves _once_ in a small code base for a personal project? Of course not.

But doing it multiple times, within a team of several developers, working on a long-term project that spans over multiple years could, and surely will, become a **huge maintainability problem**.
...
- with projects that span over **more than 6 months**;
- within larger teams, containing **at least 3 developers** working on the UI.
