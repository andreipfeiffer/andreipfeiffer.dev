...
Before discussing the solutions to CSS scalability issues, we must understand what these issues actually are. Below is a summary of the topics covered in this first chapter:
...
- [**Source order precedence**](#source-order-precedence) could render unexpected results;
...
<Figure
  width="grid"
  caption={
    "Most notable CSS scalability issues began to surface around 2003 with the shift to CSS-based layouts, and started to fadeout around 2015 when styles encapsulation became popular."
  }
>
  <ThemedImg
    light={img_light.src}
...
_"Why not before?"_, you might ask. Well, before 2003, CSS wasn't heavily used in web development. Structure and style were not separate concerns. The HTML code we wrote also included styling, which is often referred to as [tag soup](https://en.wikipedia.org/wiki/Tag_soup).
...
- For **alignment**, we used HTML attributes such `align` or `valign`;
...
But, during the same year, Simon Willinson also acknowledged that [maintaining CSS stylesheets](https://simonwillison.net/2005/Sep/26/maintainability/) is not trivial:
...
So, we can safely conclude that CSS has an inherent predisposition to problems. As soon as developers started using CSS intensively, they've also encountered scalability and maintainability problems. Therefore, let's explore the most concerning issues.
...
<Subheading>Organizing media queries</Subheading>
...
The downside of this approach is that there's a **lot of duplication in the code**, which is troublesome both during development and maintenance. Not to mention that it increases the CSS output.
...
<p>
  In <LinkTo part={2} />, we'll see how tools with support for contextual styles
  solve the problem of source code duplication.
</p>

## Naming collisions

All the CSS rules that we define or import as 3rd party CSS will end up in a single **global namespace**. Therefore the likelihood of having 2 classed with the same name scales proportionally with the size of the code.

Reusable class names usually contain common nouns, like `.modal`, `.button`, `.overlay`, and so on. If we include an external file that defines the same class names, they could get overwritten, depending on which stylesheet we include last.

<Subheading>Namespacing</Subheading>

CSS lacks support for namespaces, so the language itself doesn't help us in preventing style overwriting. A standard solution to this problem is to add a project-specific **prefix**, for instance, `.abc-overlay`. Third-party libraries usually implement this approach.

However, this does not guarantee unique names. For example, when dealing with many large files, how could we be sure that nobody else added the class `.abc-heading-large`? Of course, we could **search the entire code base** to see if we have a class with the same name already defined, but this only works for static classes.

<Note title="To keep in mind">
  There is an experimental proposal called{" "}
...
It's not unusual to deal with **dynamic** class names, that are computed by custom logic. As a consequence, this would prevent us from searching for a string like `.abc-heading-large`:
...
One way to avoid naming collisions is to increase the "strength of a selector" called [specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity). It works great in the short term, but usually gets out of control sooner or later:
...
- <LinkTo part={4} /> covers how we can maintain a low specificity by preferring
  class composition for styles overwriting.
- <LinkTo part={5} /> presents out-of-the-box solutions using utility classes.

<Note>
  Specificity is <a href="https://specificity.keegan.st/">calculated</a> based
  on CSS selector definition and it's commonly displayed as a triad like{" "}
  <code>(1,3,2)</code>.
</Note>
...
3. Working with **dynamically loaded stylesheets could render un-deterministic styles resolution**, becoming a nightmare to manage.
...
On the other hand, CSS rules and selectors work by default as **implicit dependencies** for UI components, templates, or any partial markup of an HTML page.
...
CSS code, like any other code, will increase in size indefinitely. The particular problem with CSS is that `.css` files will usually contain a certain amount of code that's not referenced anywhere in HTML.

🧟 **Unused code** is also called **zombie code** because it should be removed, killed, dead, but instead, it manages to linger around. It's not used anywhere, but it exists in an undead form. In addition, every zombie movie depicts them as being slow. The same holds true for zombie code, as it slows down the page load.
...
We avoid deleting CSS code because we have to make sure that it really is _unused_, which usually implies a great effort.
...
As you probably saw in the examples or conclude from your personal experience, none of the implementations is straightforward. They look more like workarounds instead of solid and elegant solutions. In addition:
...
- **Code editors cannot display suggestions** for defined (S)CSS variables, making them difficult to discover, especially to developers unfamiliar with the code. Some plugins attempt to support this limitation, but they are pretty flawed.
...
Let's take a look at HTML parsers for a second. We used to have [Strict Doctypes](https://24ways.org/2005/transitional-vs-strict-markup) for HTML4 and XHTML1, which enforced strict parsing for `.html` documents. However, after long battles, [HTML5 defeated XHTML2](https://www.digital-web.com/articles/html5_xhtml2_and_the_future_of_the_web/) in the popularity contest, while also dropping support for a Strict Doctype.

Therefore, we are allowed to write any gibberish code, because the parsers will make their best effort to fix any syntax errors and render something valid:
...
Similarly, CSS is also very relaxed. There never was a strict parser for CSS. So, we got used to the unsafe nature of both HTML, CSS, and JavaScript. We had to, as there was no alternative.

However, the tables had turned when static type-checkers like **TypeScript** and **Flow** became popular and made their way into UI development. The only limitation is that they provide type-safety for the markup and logic, but **not for styling**:
...
- **Editors lack productivity features support** such as auto-complete, type-checking identifiers, or discovering available CSS classes or variables. Without these features, developers are dependent on high cognitive load to either remember how CSS classes or variables are named or constantly copy & paste them.
...
In <LinkTo part={2} />, we'll cover CSS preprocessors, which allow us to keep our source code cleaner and CSS postprocessors that optimize our builds' output.
