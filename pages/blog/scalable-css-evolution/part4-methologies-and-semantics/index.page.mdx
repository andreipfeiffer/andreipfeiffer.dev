...
import img from './css-methodologies.png'
...
- [**High-level architecture**](#high-level-architecture) looks briefly at SMACCS and ITCSS as solutions for structuring application-wide CSS code;
...
<Figure background={"#fff"} width="grid">
  <Image
    ...
    width="1920"
    height="1033"
    alt="Timeline of scalable CSS evolution, highlighting popular CSS Methodologies and Architectures"
  />
</Figure>
...
1. **Separate container from content**, or in other words, the objects (or components) should provide only the abstract container, while the consumers of the objects should provide their own specific content. This way, we gain **high reusability** because each consumer of the _container_ can pass its custom _content_.
   ...
   In today's terms, the _"content"_ refers to `children` in React or `<slot>` elements in Vue.js or Web Components.
...
- [**SMACCS**](http://smacss.com/book/) is the first popular approach that addressed the topic of large-scale project-wide CSS architecture. It stands for _Scalable and Modular Architecture for CSS_ and it was coined by **Jonathan Snook** in [2011](https://snook.ca/archives/html_and_css/css-architecture).
...
3. The above layout components will contain **concrete components** (or **modules** as called in SMACCS). These are the actual UI parts, having application-specific styling including:
...
The vast majority of CSS frameworks that we know today were built on top of many of the principles described above, imbracing the semantic CSS approach. Some popular semantic CSS frameworks include _Foundation_, _Bootstrap_, _Semantic UI_, _UIKit_, _Bulma_, and many more.
...
If we get trapped inside the semantic bubble, we might get the impression that this is _"the right way"_, especially since official guides tell us so.

But when we step outside of the bubble and look objectively at Semantic CSS, we have to admit that it has some serious shortcomings:

1. There's usually **a lot of repetition** in the written CSS rules. We'll surely have lots of `display: flex`, `font-weight: bold`, or `margin: 1em` in our code, which results in larger than needed file size in the end.
...
3. Anytime a single CSS rule changes, the **cached `.css` files get invalidated**. Frequent styles updates could make the caching mechanism built in the browsers totally inefficient.
...
But, whenever there's a debatable status quo, there will always be someone to question it. The same thing happened in 2013 with Semantic CSS, when a new paradigm was born as an attempt to solve the above shortcomings. We'll cover <LinkTo part={6}>Atomic CSS</LinkTo> in a later chapter.

For now, we'll continue the Semantic CSS journey with **<LinkTo part={CURRENT_PART + 1} />**, which explores different methods to write **scoped CSS**, including **CSS Modules**. These techniques enabled us to elegantly avoid naming collisions within the global namespace, eventually becoming an industry standard.
